...
export function TeacherClassrooms() {
  ...
  useEffect(() => {
    ...
    async function loadClassrooms() {
      try {
        ...
        const response = await apiFetch<{ classrooms: Classroom[] }>("/classrooms", { token });
        if (active) {
  ...
  if (classrooms.length === 0) {
    return (
      <ActionPanel
        ...
        description="Create a classroom to invite students and begin planning sessions."
  ...
  return <ClassroomList classrooms={classrooms} detailsBasePath="/teacher/classrooms" showInviteCode />;
}
